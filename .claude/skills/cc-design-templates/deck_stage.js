 ...
 * - Auto-assigns data-screen-label to each slide
 *
 * Usage:
 *   <deck-stage>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Shadow DOM renders first (independent of children, unaffected by parser timing)
      this._render();

      // Defense: if script is in <head> (instead of after </deck-stage>),
      // parser may not have finished processing child <section>s, querySelectorAll returns empty.
      // Delay to next event loop to ensure all child nodes are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document still parsing, wait for DOMContentLoaded to handle all sections
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document parsed (script at body bottom or defer), collect in next frame
